% Script that takes linear param_table lme and returns a table with confidence
% intervals, p-values, and effect estimates according to a specified order.
% Presents effects of HII and HSCL at sample values relative to a baseline
% of 0. Used in conjunction with plotting function.
...
% Reorder names to follow order in names_array
% idx_permute = zeros(n_names, 1);
% for i = 1:n_names
%     idx_permute(i) = find(findInd(names, names_array(i, 1)));
% end
